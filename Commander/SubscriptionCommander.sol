...
contract SubscriptionCommander is Subscription,Commander {
    ...
    function requestRefund() external payable canUse() checkLive {
        ...
        if (block.timestamp > packInfo.times2 && block.timestamp < packInfo.times2 + 86400) {
            ...
            (,bytes memory resultPercent) = address(abi.decode(result,(address))).staticcall(abi.encodeWithSignature("getPercent(uint256,uint256)",packInfo.times3-packInfo.times2,packInfo.times3 - block.timestamp));
            percent = uint8(100 - abi.decode(resultPercent,(uint)));
        }
