...
contract KLA_SubscriptionCommander is Subscription,KLA_Commander {
   ...
   function requestRefund() external payable canUse() blockReEntry {
        ...
        if (isLive == 0) {
            ...
            // } else if (block.timestamp > packInfo.times2 && block.timestamp < packInfo.times2 + 300) {
                if (block.timestamp > packInfo.times2) {
                    if ( noshowLimit == 0 ) {
            ...
            // } else if ( block.timestamp > packInfo.times2 + 300 ) {
    ...
    function calculate() external payable onCaculateTime checkLive {
        ...
        uint256 balance = 0 ;
        uint256 swap = 0 ;
        if ( block.timestamp > packInfo.times3 + a ) { // caculate Manager
    ...
    function viewNoshowTime() external view returns (uint) { return noShowTime; }
